...
const StudentDashboard = () => {
    ...
    const fetchComplaints = async () => {
        try {
            ...
            if (error) throw error;
            console.log("Raw Data from Supabase:", data); // Check the console
        
            // --- CORRECTED MAPPING LOGIC START ---
            const mapped = data.map(c => {
                // Because of the UNIQUE constraint, c.feedback is likely an OBJECT, not an array.
                // But we handle both cases just to be safe.
                const feedbackData = Array.isArray(c.feedback) ? c.feedback[0] : c.feedback;
                ...
                return {
                    ...
                    dateCreated: c.created_at,
                    attachments: c.complaint_attachments,
                    // Check if feedbackData exists (is not null/undefined)
                    feedback: feedbackData
                        ? {
                            ...
                            ease_of_use_rating: feedbackData.ease_of_use_rating,
